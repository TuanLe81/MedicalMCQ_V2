...
interface AuthGuardProps {
  ...
  requireNonDemo?: boolean;
  demoLockTitle?: string;
  demoLockDescription?: string;
}
...
export function AuthGuard({
  ...
  // If feature requires a real registered user and current user is demo
  if (requireNonDemo && user.isDemo) {
    return (
      <div className="min-h-[75vh] flex items-center justify-center px-4 py-12 bg-gradient-to-b from-amber-500/5 via-background to-background">
        <div className="w-full max-w-lg p-8 sm:p-10 rounded-3xl border border-amber-300 dark:border-amber-900/70 bg-card shadow-2xl text-center space-y-6 animate-in zoom-in-95 duration-200">
          {/* Demo Lock Icon */}
          <div className="mx-auto flex h-16 w-16 items-center justify-center rounded-3xl bg-gradient-to-tr from-amber-500 to-rose-500 text-white shadow-xl shadow-amber-500/25">
            <Lock className="h-8 w-8" />
          </div>

          <div className="space-y-2">
            <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-amber-100 dark:bg-amber-950/80 border border-amber-300 dark:border-amber-800 text-amber-800 dark:text-amber-300 text-xs font-black">
              <ShieldAlert className="h-3.5 w-3.5 text-amber-600" />
              <span>TÀI KHOẢN MẪU - CHẾ ĐỘ CHỈ XEM</span>
            </div>
            <h2 className="text-2xl sm:text-3xl font-black text-foreground tracking-tight">
              {demoLockTitle}
            </h2>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-3 pt-2">
            <Link
              href="/register"
              className="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-6 py-3 rounded-2xl bg-gradient-to-r from-sky-600 to-indigo-600 hover:from-sky-700 hover:to-indigo-700 text-white font-bold text-xs sm:text-sm shadow-md shadow-sky-600/20 transition-all hover:scale-[1.02]"
            >
            ...
            <Link
              href="/folders"
              className="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-5 py-3 rounded-2xl border border-border bg-card hover:bg-muted font-bold text-xs sm:text-sm text-foreground transition-all"
            >
          ...
          <div className="border-t border-border/70 pt-4 flex items-center justify-center gap-2 text-[11px] text-muted-foreground">
            ...
            <span>Tài khoản thật sẽ có kho lưu trữ riêng biệt và tính năng chia sẻ theo email</span>
          </div>
        </div>
      </div>
    );
